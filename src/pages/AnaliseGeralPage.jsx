...
export default function AnaliseGeralPage() {
  ...
  // Paginação dos painéis de equipamentos
  const [equipVisible, setEquipVisible] = useState(5)
  ...
  // reset visible count quando os dados mudam
  useEffect(() => { setEquipVisible(5) }, [data])

  const visibleEquips = equipRows.slice(0, equipVisible)
  ...
  const maxArea  = Math.max(...visibleEquips.map(e => e.area_ha), 1)
  const maxRend  = Math.max(...visibleEquips.map(e => e.rendimento_operacional_hah), mainBenchmark?.rendimento_operacional_hah_grupo ?? 0, 0.1)
  const maxVel   = Math.max(...visibleEquips.map(e => e.velocidade_media_kmh), 0.1)
  const maxComb  = Math.max(...visibleEquips.map(e => e.consumo_medio_lh), 0.1)
  const maxTempo = Math.max(...visibleEquips.map(e => e.tempo_produtivo_h), 0.1)

  const benchRend = mainBenchmark?.rendimento_operacional_hah_grupo ?? null

  const showMoreBtn = (
    <ShowMoreBtn
      current={equipVisible}
      total={equipRows.length}
      onMore={() => setEquipVisible(v => v + 5)}
    />
  )
      ...
      {equipRows.length > 0 && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 8 }}>
            ...
            <MiniPanel title="Consumo Médio (l/h)" subtitle={`— Média: ${fmt(agg.consumo_medio_lh, 1, ' l/h')}`}>
              {visibleEquips.map(e => (
                ...
                {visibleEquips.map(e => {
                  ...
                  return (
                    <div key={e.equip} style={{ background: sc.bg, borderRadius: 6, padding: '10px 12px', textAlign: 'center' }}>
                      <div style={{ fontSize: 11, color: '#6b6560' }}>{e.label}</div>
          ...
          {/* Botão mostrar mais — aparece uma vez, fora do grid */}
          {showMoreBtn}
          <div style={{ marginBottom: 20 }} />
        </>
      )}

      {/* ── BLOCO 3: Distribuição de Tempo + Motivos de Parada ─────────── */}
